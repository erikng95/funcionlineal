     ...
     else
       {
         ...
         for (j=0;j<l;j++) /* numero de bits en la linea */
            {    
             ...
             dif = punt - punt_ant; /* diferencia entre el bit actual y el anterior */
             ...
             if (dif > 1) /* si la diferencia de lineas es mayor que 1, rellenar bits intermedios*/
             {
              dif_med = (dif/2);   
              ...
              dif_med = ((dif/2) + (dif%2));
              if((dif_med) > 1) /* si la diferencia es menos que 1, no rellenar mas bits */
               {  
                n = pasar_byte(m+1); /* si el bit intermedio esta en el siguiente byte, incrementar pointer temporalmente */          
                for(punt_inter=0;punt_inter<(dif_med-1);punt_inter++) /* rellenar puntos intermedios con la mascara del siguiente bit */
                {
                 ptr1 += bytlin;
                 yyy += bytlin;
                 if (yyy >= 0x20000)
                ...
                if((m+1)>7) /*si se ha incrementado temporalmente, retroceder el pointer*/
                {
                 ptr1--;
                 yyy--;
                 if (yyy < 0x20000)
             ...
             punt_ant = punt;
             m = pasar_byte(m); /* si es 8, pasar a 0 e incrementar pointer, sino, return m*/                       
             masc1 = tblbit[m];          
             *ptr = (*ptr & ~masc1) | masc1;
             m++;      
            }
